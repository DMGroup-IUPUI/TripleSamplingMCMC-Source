...
double graph_::getTripleWeight(graphlet* gl, struct neighbor* tempNeighbors)
{
	...
	//on test
	/*tempNeighbors->neiList.clear();
	tempNeighbors->neiList.resize(3); //three nodes three neighbor lists
	tempNeighbors->neiListEnd.clear();
	tempNeighbors->neiListEnd.resize(3); //three iterattor to the end of the list
	tempNeighbors->listOffset.clear();
	tempNeighbors->listOffset.resize(3); //three offset values to account for invalid results
	
	tempNeighbors->neiList[0].clear();
	tempNeighbors->neiList[1].clear();
	tempNeighbors->neiList[2].clear();

	tempNeighbors->neiList[0].resize(getMaxAdjLen()*2); //maximum size of the vector needed. should increase efficiency
	tempNeighbors->neiList[1].resize(getMaxAdjLen()*2);
	tempNeighbors->neiList[2].resize(getMaxAdjLen()*2);*/
	//on test
...
unsigned int graph_::nodeDegree(unsigned int node)
{
	return ADJIndexEnd[node] - ADJIndexStart[node];
}
