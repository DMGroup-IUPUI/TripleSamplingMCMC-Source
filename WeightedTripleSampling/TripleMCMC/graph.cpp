...
void graph_::populate_neighbors(graphlet* gl,  neighbor* Neighbors){
	int v0,v1,v2;
	v0=gl->vertices[0];
	v1=gl->vertices[1];
	v2=gl->vertices[2];
...
double graph_::getNodeWeight(unsigned int node,  neighbor* tempNeighbors)
{
	double w = 0;
	double t = 0;
...
void graph_::printTriplesOfNode(unsigned int node,  neighbor* tempNeighbors)//print weights of all triple of a node
{
	double t = 0;
	graphlet *gl = new graphlet(-1,-2,-3);
	unsigned int s1,s2,e1;
...
void graph_::sampleTripleFromNode(unsigned int v0, double rand, graphlet *tgl, neighbor* tempNeighbors)
{
	double w = 0;
	unsigned int s1,s2,e1;
	s1 = ADJIndexStart[v0];
...
double graph_::getTripleWeight(graphlet* gl,  neighbor* tempNeighbors)
{
	unsigned int n,d;
...
unsigned int graph_::totalNeighbors( neighbor* tempNeighbors)
{
	unsigned int n =0 ;
...
unsigned int graph_::randAdjNode(unsigned int vc)
{
	unsigned int rand = boost_get_a_random_number(ADJIndexStart[vc],ADJIndexEnd[vc]);
	return ADJ[rand];
}

void graph_::randomTriple(graphlet* triple)//assigns a random triple to the "triple"
{
	//select 1st node
	unsigned int v0 = boost_get_a_random_number(0,getVertexCount());
	while (ADJIndexEnd[v0]-ADJIndexStart[v0]<2) {
		v0 = boost_get_a_random_number(0,getVertexCount());
	}
	
	//select 2nd node
	unsigned int v1 = boost_get_a_random_number(ADJIndexStart[v0],ADJIndexEnd[v0]);
	v1 = ADJ[v1];
	
	//select 3rd node
	unsigned int v2 = boost_get_a_random_number(ADJIndexStart[v0],ADJIndexEnd[v0]);
	v2 = ADJ[v2];
	while (v1==v2) {
		v2 = boost_get_a_random_number(ADJIndexStart[v0],ADJIndexEnd[v0]);
		v2 = ADJ[v2];
	}
	
	triple->graphletReplace(v0,v1,v2);
}
